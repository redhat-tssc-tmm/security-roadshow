...
echo -n "---> Getting tools and versions <---"
...
if command -v podman &>/dev/null; then
  PODMAN_VER=$(podman --version | grep -Eo 'v?[0-9]+\.[0-9]+\.[0-9]+')
  append_ver "podman    |${PODMAN_VER#v}     |Free & Open Source Container Tools"
fi

if command -v git &>/dev/null; then
  GIT_VER=$(git --version | grep -Eo 'v?[0-9]+\.[0-9]+\.[0-9]+')
...
echo -ne "\r\033[K"  # Clear the status line
